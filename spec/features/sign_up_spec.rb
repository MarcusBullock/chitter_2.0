feature 'user sign up' do
  scenario 'a user may sign up to Chitter' do
    ...
    expect(page).to have_content "marc_b's Chitter!"
    expect(User.first.email).to eq('marcb@hotmail.com')
  end
  ...
  scenario 'email must be correct format' do
    expect{ sign_up(email: 'fake@fake') }.not_to change(User, :count)
    expect(current_path).to eq('/users/new')
    expect(page).to have_content 'Email has an invalid format'
  end

  scenario 'password confirmation must match' do
      expect{ sign_up(password_confirmation: '123password') }.not_to change(User, :count)
